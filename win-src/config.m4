...
dnl PHP_ARG_ENABLE(xapm, whether to enable xapm support,
dnl Make sure that the comment is aligned:
dnl [  --enable-xapm           Enable xapm support])
...
if test "$PHP_XAPM" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $XAPM_DIR/lib, XAPM_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_XAPMLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong xapm lib version or lib not found])
  dnl ],[
  dnl   -L$XAPM_DIR/lib -lm
  dnl ])
  dnl
  dnl PHP_SUBST(XAPM_SHARED_LIBADD)

  PHP_NEW_EXTENSION(xapm, xapm.c, $ext_shared)
fi
