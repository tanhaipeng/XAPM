/*
  ...
  +----------------------------------------------------------------------+
  | Author: AllinGoo(tanhp@outlook.com)                                  |
  +----------------------------------------------------------------------+
*/
...
/* If you declare any globals in php_xapm.h uncomment this:
...
PHP_INI_BEGIN()
    STD_PHP_INI_ENTRY("xapm.global_value",      "42", PHP_INI_ALL, OnUpdateLong, global_value, zend_xapm_globals, xapm_globals)
    STD_PHP_INI_ENTRY("xapm.global_string", "foobar", PHP_INI_ALL, OnUpdateString, global_string, zend_xapm_globals, xapm_globals)
PHP_INI_END()
*/
/* }}} */

/* {{{ php_xapm_init_globals
 */
...
PHP_FUNCTION(confirm_xapm_compiled)
{
	...
	len = spprintf(&strg, 0, "Congratulations! You have successfully modified ext/%.78s/config.m4. Module %.78s is now compiled into PHP.", "xapm", arg);
